...
function equipoMarco(noEquipo, puntos, jugador) {
  ...
  document.getElementById(noEquipo === 1 ? "marcadorEquipo1" : "marcadorEquipo2").innerHTML =
    (noEquipo === 1 ? marcadorEquipo1 : marcadorEquipo2);

  let arregloJugadores = (noEquipo === 1 ? jugadoresEquipo1 : jugadoresEquipo2);
  let numPlayera = traeNumPlayera(arregloJugadores, jugador);
  let jugadorAnotador = `
<tr>
  ...
  `;
  let htmlMarcadorJugadoresAnotadores = document.getElementById(noEquipo === 1 ? "listaAnotadoresEquipo1" : "listaAnotadoresEquipo2");
  htmlMarcadorJugadoresAnotadores.innerHTML = htmlMarcadorJugadoresAnotadores.innerHTML + jugadorAnotador;
}
...
function ponerJugadores(noEquipo) {
  ...
  let htmlElementoEquipoNombre;
  let htmlElementoMarcadorEquipoNombre;
  let jugadoresEquipo;
  let nombreEquipo;
  if (noEquipo === 1) {
    jugadoresEquipo = jugadoresEquipo1;
    htmlElementoJugadores = document.getElementById("lista-equipo1");
    htmlElementoEquipoNombre = document.getElementById("equipo1nombre");
    htmlElementoMarcadorEquipoNombre = document.getElementById("equipo1AnotadoresNombre");
    nombreEquipo = equipo1;
  } else {
    jugadoresEquipo = jugadoresEquipo2;
    htmlElementoJugadores = document.getElementById("lista-equipo2");
    htmlElementoEquipoNombre = document.getElementById("equipo2nombre");
    htmlElementoMarcadorEquipoNombre = document.getElementById("equipo2AnotadoresNombre");
    nombreEquipo = equipo2;
  }
  htmlElementoMarcadorEquipoNombre.innerText = nombreEquipo;
  htmlElementoEquipoNombre.innerText = nombreEquipo;
  let jugadores = "";
  for (i = 0; i < jugadoresEquipo.length; i++) {
    let numPlayera = "";
    if (jugadoresEquipo[i][1] != undefined) {
      numPlayera = jugadoresEquipo[i][1];
    } else {
      numPlayera = "--";
    }
    jugadores += "<tr>" +
